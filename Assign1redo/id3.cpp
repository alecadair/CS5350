...
ID3::ID3(vector<vector<string> > _names, vector<vector<bool> > _attribute_table,vector<bool> _labels){
	names_vector = _names;
...
Node ID3::id3_algorithm(vector<vector<string> > names,
					...
					vector<bool> labels, vector<int> _skip_list){
	...
	for(unsigned i = 0; i < 2; i++){
		...
		if(sv_size == 0){
			...
			unsigned pos_count, neg_count = 0;
			for(unsigned j = 0; j < names.size(); j++){
				if(labels[j] == true)					pos_count ++;
				else
					neg_count ++;
			}
			if(pos_count > neg_count)
			...
			new_root.children.push_back(id3_algorithm(names_sv,attribute_table_sv,labels_sv, skip_list));
		}
	}
	return new_root;
}
...
double ID3:: calculate_info_gain(vector<vector<bool> > attribute_table,
										vector<bool> labels,
										vector<int> skip_list, int attribute){
	double total_entropy = calculate_entropy(attribute_table,labels, true,skip_list,attribute,false);
	...
	double entropy_true = calculate_entropy(attribute_table_truesv,labels_truesv,false,
											skip_list,attribute,true);

	double entropy_false = calculate_entropy(attribute_table_falsesv, labels_falsesv, false,
											skip_list,attribute,false);
	entropy_true *= (((float)labels_truesv.size())/((float)labels.size()));
	entropy_false *= (((float)labels_falsesv.size())/((float)labels.size()));
	double gain = total_entropy -(entropy_true + entropy_false);
	return gain;
}

double ID3:: calculate_entropy(vector<vector<bool> > attribute_table,
								vector<bool> labels, bool all_labels,
								vector<int> skip_list, int attribute, bool attribute_val){
