		...
		if(dot_prod <= 1){
			...
			while(weights_iter != weights.end()){
				...
				if(search != example.end())
					weights_iter->second += example.find(ind)->second;
				weights_iter ++;
...
double SVM:: dot_product(map<unsigned int, double> v1, map<unsigned int, double> w){
...
double SVM:: get_example_from_data(string line, map<unsigned int, double>* feat_vec, double* label){
...
void SVM::fill_training_vecs(string filename, char is_test){
    ...
    if(train_stream.is_open()){
        while(getline(train_stream,train_line)){
            ...
            double label = 0;
            get_example_from_data(train_line, &feat_vec, &label);
	    if(!is_test){
